...
public class CloudAnchorActivity extends AppCompatActivity
  ...
  private final ObjectRenderer virtualObject1 = new ObjectRenderer();
  private final ObjectRenderer virtualObjectShadow1 = new ObjectRenderer();
  ...
  private final Object anchorLock1 = new Object();
  private final Object anchorLock2 = new Object();
  ...
  @GuardedBy("anchorLock1")
  private Anchor anchor1;
  @GuardedBy("anchorLock2")
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    myButton.setOnClickListener((view) -> onMyButtonPress());
  ...
  public void onDrawFrame(GL10 gl) {
    ...
    try {
      ...
      synchronized (anchorLock1) {
        if (anchor1 != null && anchor1.getTrackingState() == TrackingState.TRACKING) {
          // Get the current pose of an Anchor in world space. The Anchor pose is updated
          // during calls to session.update() as ARCore refines its estimate of the world.
          anchor1.getPose().toMatrix(anchorMatrix, 0);
          shouldDrawAnchor = true;
        }
      }
      ...
      shouldDrawAnchor = false;
      synchronized (anchorLock2) {
        if (anchor2 != null && anchor2.getTrackingState() == TrackingState.TRACKING) {
          // Get the current pose of an Anchor in world space. The Anchor pose is updated
          // during calls to session.update() as ARCore refines its estimate of the world.
          anchor2.getPose().toMatrix(anchorMatrix, 0);
          shouldDrawAnchor = true;
        }
      }
  ...
  private void setNewAnchor(Anchor newAnchor,long roomCode) {
  ...
  private void onMyButtonPress() {
    ...
    firebaseManager.registerNewListenerForRoom(
            ...
            cloudAnchorId -> {
              // When the cloud anchor ID is available from Firebase.
              CloudAnchorResolveStateListener resolveListener =
                      new CloudAnchorResolveStateListener(15L/*roomCode*/);
              Preconditions.checkNotNull(resolveListener, "The resolve listener cannot be null.");
              cloudManager.resolveCloudAnchor(
                      cloudAnchorId, resolveListener, SystemClock.uptimeMillis(),2);
            });
  }
  ...
  private void onRoomCodeEntered(Long roomCode) {
    ...
    firebaseManager.registerNewListenerForRoom(
        ...
        cloudAnchorId -> {
          ...
          Preconditions.checkNotNull(resolveListener, "The resolve listener cannot be null.");
          cloudManager.resolveCloudAnchor(
              cloudAnchorId, resolveListener, SystemClock.uptimeMillis(),0);
        });
  }

  /** Callback function invoked when the user presses the OK button in the Resolve Dialog. */
  private void loadRoomCode(/*Long roomCode*/) {
    currentMode = HostResolveMode.RESOLVING;
    hostButton.setEnabled(false);
    resolveButton.setText(R.string.cancel);
    roomCodeText.setText(String.valueOf(99L/*roomCode*/));
    snackbarHelper.showMessageWithDismiss(this, getString(R.string.snackbar_on_resolve));

    FirebaseManager firebaseManager1 = new FirebaseManager(this);
    // Register a new listener for the given room.
    firebaseManager1.registerNewListenerForRoom(
            11L/*roomCode*/,
            cloudAnchorId -> {
              // When the cloud anchor ID is available from Firebase.
              CloudAnchorResolveStateListener resolveListener =
                      new CloudAnchorResolveStateListener(11L/*roomCode*/);
              Preconditions.checkNotNull(resolveListener, "The resolve listener cannot be null.");
              cloudManager.resolveCloudAnchor(
                      cloudAnchorId, resolveListener, SystemClock.uptimeMillis(),1);
            });

    FirebaseManager firebaseManager2 = new FirebaseManager(this);
    // Register a new listener for the given room.
    firebaseManager2.registerNewListenerForRoom(
            12L/*roomCode*/,
            cloudAnchorId -> {
              // When the cloud anchor ID is available from Firebase.
              CloudAnchorResolveStateListener resolveListener =
                      new CloudAnchorResolveStateListener(12L/*roomCode*/);
              Preconditions.checkNotNull(resolveListener, "The resolve listener cannot be null.");
              cloudManager.resolveCloudAnchor(
                      cloudAnchorId, resolveListener, SystemClock.uptimeMillis(),2);
            });
  ...
  private final class CloudAnchorResolveStateListener
    ...
    @Override
    public void onCloudTaskComplete(Anchor anchor,long roomCode) {
